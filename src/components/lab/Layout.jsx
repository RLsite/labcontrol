...
import { FlaskConical, LayoutDashboard, ShieldCheck, LogOut, CalendarDays } from "lucide-react";
...
export default function Layout({ user, isAdmin, profile, children }) {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-50 text-slate-900 font-body">
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <div className="text-sm font-semibold text-slate-900">מעבדת חומרים מתקדמים</div>
              <div className="text-[11px] text-slate-500">ניהול מכשירים ובקרת גישה</div>
            </div>
          </div>

          <nav className="hidden md:flex items-center gap-1">
            {navItem("/", "לוח בקרה", LayoutDashboard)}
            {navItem("/schedule", "לוח זמנים", CalendarDays)}
            {isAdmin && navItem("/admin", "ניהול", ShieldCheck)}
          </nav>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex flex-col items-end leading-tight">
              ...
              <span className="text-[11px] text-slate-500">
                {isAdmin ? "מנהל מעבדה" : profile?.status === "approved" ? "חוקר מאושר" : "משתמש"}
              </span>
            </div>
        ...
        <div className="md:hidden flex items-center gap-1 px-4 pb-2">
          {navItem("/", "בקרה", LayoutDashboard)}
          {navItem("/schedule", "זמנים", CalendarDays)}
          {isAdmin && navItem("/admin", "ניהול", ShieldCheck)}
        </div>
      </header>
      ...
      <footer className="max-w-7xl mx-auto px-4 sm:px-6 py-6 text-center text-[11px] text-slate-400">
        מערכת ניהול מכשירי מעבדה · כל הזכויות שמורות
      </footer>
    </div>
  );
}