...
export default function ReservationModal({ devices, user, onClose, onSaved }) {
  ...
  const gcalLink = done
    ? googleCalendarUrl({
        title: `${lang === "he" ? "הזמנה" : "Reservation"}: ${selectedDevice?.name || ""}`,
        startISO: done.startISO,
        endISO: done.endISO,
        details: purpose ? `${t("reserve.purpose")}: ${purpose}` : "",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" dir={lang === "he" ? "rtl" : "ltr"}>
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center">
              <CalendarPlus className="w-5 h-5 text-indigo-600" />
            </div>
            <h3 className="font-bold text-slate-900">{t("reserve.title")}</h3>
          </div>
        ...
        {done ? (
          <div className="p-6 text-center">
            ...
            <h4 className="font-bold text-slate-900 text-lg">{t("reserve.saved")}</h4>
            <p className="text-sm text-slate-500 mt-1">
              {selectedDevice?.name} · {formatDateTime(done.startISO, lang)}
            </p>
            <a href={gcalLink} target="_blank" rel="noreferrer">
              <Button className="mt-5 w-full gap-2 bg-indigo-600 hover:bg-indigo-700 text-white">
                ...
                {t("reserve.syncGoogle")}
              </Button>
            </a>
            <p className="text-[11px] text-slate-400 mt-2">{t("reserve.syncNote")}</p>
            <Button variant="ghost" className="mt-3 w-full" onClick={onClose}>{t("common.close")}</Button>
          </div>
        ) : (
          <form onSubmit={submit} className="p-5 space-y-4">
            ...
            <Field label={t("reserve.purpose")}>
              <textarea
                ...
                rows={2}
                placeholder={t("reserve.purposePlaceholder")}
            ...
            <div className="flex gap-2 pt-2">
              <Button type="submit" disabled={saving} className="flex-1 gap-2 bg-indigo-600 hover:bg-indigo-700 text-white">
                ...
                {saving ? t("reserve.creating") : t("reserve.create")}
              </Button>
